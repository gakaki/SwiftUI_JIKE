...
public class AZTabBarController: UIViewController {
    ...
    open fileprivate(set) var isAnimating: Bool = false
    ...
    public fileprivate(set) var controllersContainer: UIView!

    /// The view that holds the stack and selection indicator
    public fileprivate(set) var buttonsContainer: UIView!
    ...
    public internal(set) var selectionIndicator: UIView!
    ...
    fileprivate lazy var buttonsText: [String?] = [String?](repeating: nil, count: self.tabCount)

    fileprivate lazy var buttonsColors: [UIColor?] = [UIColor?](repeating: nil, count: self.tabCount)
    ...
    public override func loadView() {
        ...
        buttonsContainer.topAnchor.constraint(equalTo: controllersContainer.bottomAnchor).isActive = true
        buttonsContainer.topAnchor.constraint(equalTo: separatorLine.topAnchor).isActive = true
        buttonsContainerHeightConstraint = buttonsContainer.heightAnchor.constraint(equalToConstant: 50)
    ...
    public init(withTabIcons tabIcons: [UIImage], highlightedIcons: [UIImage]? = nil) {
        ...
        initialize(withTabIcons: tabIcons, highlightedIcons: highlightedIcons)
    }
    ...
    public override func viewDidLoad() {
        super.viewDidLoad()
        buttonsContainerHeightConstraintInitialConstant = buttonsContainerHeightConstraint.constant
    }
    ...
    public override func didRotate(from _: UIInterfaceOrientation) {
        let selectedButtonX: CGFloat = buttons[self.selectedIndex].frame.origin.x
        selectionIndicatorLeadingConstraint.constant = selectedButtonX
    }
    ...
    open func setViewController(_ controller: UIViewController, atIndex index: Int) {
        ...
        if index == selectedIndex {
            ...
            moveToController(at: index, animated: false)
        }
    }
    ...
    open func setIndex(_ index: Int, animated: Bool = true) {
        ...
        if selectedIndex != index { moveToController(at: index, animated: animated) }
    ...
    open func setBadgeText(_ text: String?, atIndex index: Int) {
        if let buttons = buttons {
            if index < buttons.count {
                notificationBadgeAppearance.distanceFromCenterX = 15
                notificationBadgeAppearance.distanceFromCenterY = -10
                let button = buttons[index] as! AZTabBarButton
                button.addBadge(text: text, appearance: notificationBadgeAppearance)
            }
        } else {
            badgeValues[index] = text
        }
    }
    ...
    open func highlightButton(atIndex index: Int) {
        highlightedButtonIndexes.add(index)
        updateInterfaceIfNeeded()
    }
    ...
    open func removeHighlight(atIndex index: Int) {
        highlightedButtonIndexes.remove(index)
        updateInterfaceIfNeeded()
    }
    ...
    open func insertTab(atIndex index: Int, icon: UIImage, selectedIcon: UIImage? = nil,
        ...
        // shift icons array
        tabIcons.insert(icon, at: index)

        // shift selected icons array
        selectedTabIcons?.insert(selectedIcon ?? icon, at: index)

        // shift controllers array
        controllers.insert(nil, at: index)

        // shift actions array
        actions.insert(nil, at: index)
    ...
    open func setTabIcon(forIndex index: Int, normal: UIImage, selected: UIImage? = nil) {
        ...
        selectedTabIcons?[index] = selected ?? normal
        updateInterfaceIfNeeded()
    }
    ...
    private func initialize(withTabIcons tabIcons: [UIImage], highlightedIcons: [UIImage]? = nil) {
        ...
        badgeValues = [String?](repeating: nil, count: tabIcons.count)

        self.tabIcons = tabIcons

        selectedTabIcons = highlightedIcons

        controllers = [UIViewController?](repeating: nil, count: tabIcons.count)

        actions = [AZTabBarAction?](repeating: nil, count: tabIcons.count)

        highlightedButtonIndexes = NSMutableSet()

        selectedIndex = 0

        separatorLineColor = UIColor.lightGray
    ...
    private func setupButtons() {
        if buttons == nil {
            buttons = [UIButton]() // NSMutableArray(capacity: self.tabIcons.count)

            for i in 0 ..< tabIcons.count {
                let button: UIButton = createButton(forIndex: i)
        ...
        buttonsContainer.backgroundColor = buttonsBackgroundColor != nil ? buttonsBackgroundColor : UIColor.lightGray
    }

    private func customizeButtons() {
        for i in 0 ..< tabIcons.count {
            let button: AZTabBarButton = buttons[i] as! AZTabBarButton

            let isHighlighted = highlightedButtonIndexes.contains(i)
            ...
            if isHighlighted {
                color = highlightedBackgroundColor ?? UIColor.black
            } else {
                color = buttonsColors[i] ?? selectedColor ?? UIColor.black
            }
            ...
            button.customizeForTabBarWithImage(tabIcons[i],
                                               highlightImage: highlightedImage,
                                               selectedColor: color,
                                               highlighted: isHighlighted,
                                               defaultColor: defaultColor ?? UIColor.gray,
                                               highlightColor: buttonsColors[i] ?? highlightColor ?? UIColor.white,
    ...
    private func moveToController(at index: Int, animated: Bool) {
        if let controller = controllers[index], !(animated && isAnimating) {
            ...
            for i in 0 ..< tabIcons.count {
                ...
                if highlightsSelectedButton, !(actions[i] != nil && controllers[i] != nil) {
            ...
            setupConstraints(forChildController: controller)
            ...
            if let currentViewControllerView = currentViewControllerView, animated, animateTabChange {
                ...
                let offset: CGFloat = view.frame.size.width / 5
                ...
                moveSelectionIndicator(toIndex: index, animated: animated)
            } else {
                moveSelectionIndicator(toIndex: index, animated: false)
            }

            selectedIndex = index
            delegate?.tabBar(self, didMoveToTabAtIndex: index)
            statusBarStyle = delegate?.tabBar(self, statusBarStyleForIndex: index) ?? .default
        }
    }
    ...
    private func setupSelectionIndicator() {
        if selectionIndicator == nil {
            ...
            buttonsContainer.addSubview(selectionIndicator)
            setupSelectionIndicatorConstraints()
        }
        selectionIndicatorHeightConstraint.constant = selectionIndicatorHeight
        selectionIndicator.backgroundColor = selectionIndicatorColor ?? UIColor.black
    }

    private func setupSeparatorLine() {
        separatorLine.backgroundColor = separatorLineColor
        separatorLine.isHidden = !separatorLineVisible
...
extension AZTabBarController: AZTabBarButtonDelegate {
    public func beginAnimationDuration(_: AZTabBarButton) -> TimeInterval {
        return iconStartAnimationDuration
    }

    public func endAnimationDuration(_: AZTabBarButton) -> TimeInterval {
        return iconEndAnimationDuration
    }

    public func usingSpringWithDamping(_: AZTabBarButton) -> CGFloat {
        return iconSpringWithDamping
    }

    public func initialSpringVelocity(_: AZTabBarButton) -> CGFloat {
        return iconInitialSpringVelocity
    }

    public func longClickTriggerDuration(_: AZTabBarButton) -> TimeInterval {
        return longClickTriggerDuration
    }

    public func longClickAction(_ tabBarButton: AZTabBarButton) {
        longClick(sender: tabBarButton)
    }
    ...
    public func shouldAnimate(_ tabBarButton: AZTabBarButton) -> Bool {
        if tabBarButton.tag == selectedIndex || highlightedButtonIndexes.contains(tabBarButton.tag) || isAnimating {
            return false
        }
...
private extension AZTabBarController {
